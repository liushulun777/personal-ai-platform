<template>
  <div>
    ...
    <n-card style="margin-bottom: 16px">
      <n-form inline :model="queryForm">
        <n-form-item label="名称" label-width="60" style="width: 220px;">
          <n-input v-model:value="queryForm.name" placeholder="请输入大屏名称" clearable />
        </n-form-item>
    ...
    <!-- 新增弹窗 -->
    <n-modal v-model:show="showModal" title="新增大屏" preset="dialog" style="width: 600px">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="80">
        <n-form-item label="名称" path="name">
          <n-input v-model:value="formData.name" placeholder="请输入大屏名称" />
        </n-form-item>
...
const columns: DataTableColumns<any> = [
  ...
  { title: '操作', key: 'actions', width: 250, render: (row) => h('div', { style: 'display: flex; gap: 8px;' }, [
    ...
    h(NButton, { size: 'small', onClick: () => handleShare(row) }, { default: () => '分享' }),
...
const formData = reactive({
  name: '',
  config: {
    canvas: { width: 1920, height: 1080, background: '#0a1628' },
    components: []
  },
  remark: ''
})
...
const fetchList = async () => {
  loading.value = true
  try {
    const data: any = await get('/report/dashboards', queryForm)
...
const handleShare = async (row: any) => {
  try {
    const data: any = await post(`/report/dashboards/${row.id}/share`)
    if (data) {
      const url = `${window.location.origin}/dashboard/preview/${data}`
...
const handleDelete = (row: any) => {
  dialog.warning({
    title: '确认删除',
    content: `确定要删除大屏 "${row.name}" 吗？`,
    onPositiveClick: async () => {
      try {
        await del(`/report/dashboards/${row.id}`)
...
const handleSubmit = async () => {
  try {
    ...
    const data: any = await post('/report/dashboards', formData)
    message.success('创建成功')
    showModal.value = false
    if (data) {
      router.push(`/dashboard/designer/${data}`)
    } else {
      fetchList()
    }
  } catch {} finally {
